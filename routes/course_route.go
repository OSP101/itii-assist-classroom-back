...
func SetupCourseRoutes(app *fiber.App, auditLogger *services.AuditLogger) {
	...
	protected.Get("/:id/overview", middlewares.RequireRole("admin", "instructor", "ta"), middlewares.RequireCourseAccess(middlewares.CourseIDFromParam("id"), "instructor", "ta"), middlewares.AuditCourseView(services.ActionViewCourse, middlewares.CourseIDFromParam("id")), handlers.GetCourseOverviewHandler)
	protected.Get("/:id", middlewares.RequireRole("admin", "instructor", "ta"), middlewares.RequireCourseAccess(middlewares.CourseIDFromParam("id"), "instructor", "ta"), middlewares.AuditCourseView(services.ActionViewCourse, middlewares.CourseIDFromParam("id")), handlers.GetCourseByIDHandler)
	...
	protected.Get("/:id/sections/:sectionId/students", middlewares.RequireRole("admin", "instructor", "ta"), middlewares.RequireCourseAccess(middlewares.CourseIDFromParam("id"), "instructor", "ta"), middlewares.RequireCoursePermission(middlewares.CourseIDFromParam("id"), repositories.PermissionViewSections, "instructor", "ta"), middlewares.AuditCourseView(services.ActionViewRoster, middlewares.CourseIDFromParam("id"), middlewares.WithViewTarget("section", "sectionId")), handlers.GetSectionStudentsHandler)
