...
func SetupScoreRoutes(app *fiber.App, auditLogger *services.AuditLogger) {
	...
	api.Get("/summary", middlewares.RequireCourseAccess(middlewares.CourseIDFromQuery("course_id"), "instructor", "ta"), middlewares.RequireCoursePermission(middlewares.CourseIDFromQuery("course_id"), repositories.PermissionViewScoreSummary, "instructor", "ta"), middlewares.AuditCourseView(services.ActionViewScores, middlewares.CourseIDFromQuery("course_id")), handlers.GetStudentScoresSummaryHandler)
	api.Get("/matrix", middlewares.RequireCourseAccess(middlewares.CourseIDFromQuery("course_id"), "instructor", "ta"), middlewares.RequireCoursePermission(middlewares.CourseIDFromQuery("course_id"), repositories.PermissionViewScoreSummary, "instructor", "ta"), middlewares.AuditCourseView(services.ActionViewScores, middlewares.CourseIDFromQuery("course_id")), handlers.GetScoreMatrixHandler)
