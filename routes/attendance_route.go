...
func SetupAttendanceRoutes(app *fiber.App, auditLogger *services.AuditLogger) {
	...
	// คำขอลา (ผู้สอน/TA) ต้องประกาศก่อน /:id ไม่งั้น "leave-requests" จะถูกจับเป็น session id
	leaveReq := middlewares.CourseIDFromLeaveRequestParam("id")
	api.Get("/leave-requests", middlewares.NoStore(), middlewares.RequireCourseAccess(middlewares.CourseIDFromQuery("course_id"), "instructor", "ta"), middlewares.RequireCoursePermission(middlewares.CourseIDFromQuery("course_id"), repositories.PermissionViewAttendance, "instructor", "ta"), handlers.ListLeaveRequestsHandler)
	api.Get("/leave-requests/count", middlewares.NoStore(), middlewares.RequireCourseAccess(middlewares.CourseIDFromQuery("course_id"), "instructor", "ta"), middlewares.RequireCoursePermission(middlewares.CourseIDFromQuery("course_id"), repositories.PermissionViewAttendance, "instructor", "ta"), handlers.CountLeaveRequestsHandler)
	api.Post("/leave-requests/batch-review", attendanceHandler.BatchReviewLeaveRequests)
	api.Get("/leave-requests/:id", middlewares.NoStore(), middlewares.RequireCourseAccess(leaveReq, "instructor", "ta"), middlewares.RequireCoursePermission(leaveReq, repositories.PermissionViewAttendance, "instructor", "ta"), handlers.GetLeaveRequestHandler)
	api.Get("/leave-requests/:id/evidence/:file", middlewares.RequireCourseAccess(leaveReq, "instructor", "ta"), middlewares.RequireCoursePermission(leaveReq, repositories.PermissionViewAttendance, "instructor", "ta"), handlers.GetLeaveEvidenceHandler)
	api.Post("/leave-requests/:id/review", middlewares.RequireCourseAccess(leaveReq, "instructor", "ta"), middlewares.RequireCoursePermission(leaveReq, repositories.PermissionReviewLeaveRequests, "instructor", "ta"), attendanceHandler.ReviewLeaveRequest)
	api.Post("/leave-requests/:id/revoke", middlewares.RequireCourseAccess(leaveReq, "instructor", "ta"), middlewares.RequireCoursePermission(leaveReq, repositories.PermissionReviewLeaveRequests, "instructor", "ta"), attendanceHandler.RevokeLeaveRequest)
	...
	api.Get("/course-summary", middlewares.RequireCourseAccess(middlewares.CourseIDFromQuery("course_id"), "instructor", "ta"), middlewares.RequireCoursePermission(middlewares.CourseIDFromQuery("course_id"), repositories.PermissionViewAttendance, "instructor", "ta"), middlewares.AuditCourseView(services.ActionViewAttendance, middlewares.CourseIDFromQuery("course_id")), handlers.GetAttendanceCourseSummaryHandler)
	...
	api.Get("/:id/records", middlewares.RequireCourseAccess(middlewares.CourseIDFromAttendanceSessionParam("id"), "instructor", "ta"), middlewares.RequireCoursePermission(middlewares.CourseIDFromAttendanceSessionParam("id"), repositories.PermissionViewAttendance, "instructor", "ta"), middlewares.AuditCourseView(services.ActionViewAttendance, middlewares.CourseIDFromAttendanceSessionParam("id"), middlewares.WithViewTarget("attendance_session", "id")), handlers.GetAttendanceRecordsHandler)
	api.Get("/:id/security-flags", middlewares.RequireCourseAccess(middlewares.CourseIDFromAttendanceSessionParam("id"), "instructor", "ta"), middlewares.RequireCoursePermission(middlewares.CourseIDFromAttendanceSessionParam("id"), repositories.PermissionViewAttendance, "instructor", "ta"), handlers.GetAttendanceSessionSecurityFlagsHandler)
	api.Get("/:id/records/:recordId/history", middlewares.RequireCourseAccess(middlewares.CourseIDFromAttendanceSessionParam("id"), "instructor", "ta"), middlewares.RequireCoursePermission(middlewares.CourseIDFromAttendanceSessionParam("id"), repositories.PermissionViewAttendance, "instructor", "ta"), handlers.GetAttendanceRecordHistoryHandler)
